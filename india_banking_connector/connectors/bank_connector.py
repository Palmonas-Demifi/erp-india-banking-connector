...
class BankConnector(Document):
    ...
    def aes_encrypt_data(self, data, key, iv=None, prepend_iv=False):
        if isinstance(data, dict):
        ...
        _iv = iv if iv else self.IV
        if isinstance(_iv, str):
            _iv = _iv.encode("utf-8")
        ...
        if prepend_iv:
            encrypted = _iv + encrypted

        return b64encode(encrypted).decode("utf-8")

    def aes_decrypt_data(self, data, key, iv=None, json_loads=True, extract_iv=False):
        if isinstance(key, str):
            key = key.encode("utf-8")
        ...
        try:
            ...
            if extract_iv and len(unpadded) > 16:
                # ICICI step 6: Ignore the first 16 bytes in the output of step 5 because it is IV.
                unpadded_json = unpadded[16:]
                try:
                    return json.loads(unpadded_json.decode("utf-8"))
                except ValueError:
                    pass
            ...
            error = e
            try:
                # ignoring IV (if included in decrypted data) fallback
        ...
        if error:
            frappe.throw(title="Decryption Failed", msg=str(error))
